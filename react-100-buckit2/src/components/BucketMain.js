...
//state 만들거나, state변화를 감시하거나 component 가 rendering 되는
//시점에 무언가를 실행시키거나 하는 용도의 함수들이다
...
/*
component의 본문만들기 함수를 선언하고 함수에서 컴포넌트
        코드를 return 한다. 이때 return 내의 컴포넌트 tag의 가장
        바깥을 감싸서 한개의 tag로 묶어주어야한다
*/
const BucketMain = () => {
  /*buckList state 변수를 배열로 선언하기
  state변수를 배열로 선언하여 사용할 경우 표준 JS코드에 있는 배열에
  요소를 추가하는 함수를 사용할수 없다 state 배열에 요소를 추가하는 경우는
  setter 함수를 사용하는데 이 함수가 작동되는 방식은
  기존의 배열을 새로운 배열로 복사하고, 요소를 추가한 후 새로운 배열과
  기존의 배열을 교환하는 방식으로 이루어진다.
  */
  const [bucketList, setBucketList] = useState(() => {
    /*
    현재 브라우저의 localStorage 에 BUCKLIST 이름으로 데이터가 저장되어 있으면
    데이터를 읽어서 JSONIN데이터로 변환 한 후 bucketList 를 생성하고
    없으면  빈 배열을 생성하라.
    */
  ...
  /*
  bucketList 데이터가 추가,수정,삭제 가되면 localStrorage 에 BUCKTLIST라능
  이름으로 JSON 데이터를 문자열로 변환하여 저장
  */
  ...
  //버킷을 입력하고 버킷을 추가할때 사용하는 함수
  const onKeyDown = (e) => {
    if (e.keyCode === 13) {
      const bucket = {
        b_id: uuid(), //uuid를 사용하여 Bucket에 PK생성
        //중요도를 표시하는 데이터로 사용
        b_flag: 0,
  ...
  const bucket_flag_toggle = (id) => {
    //map(forEach)을 사용하여 전체 리스트를 반복하면서 id가 일치하는
    //항목을 찾아 flag 값을 변경할 것이다
    const bucketBody = bucketList.map((bucket) => {
      if (bucket.b_id === id) {
        //원래의 bucket 데이터를 spread(펼치기)하고
        //b_flag 값만 원의 b_flag 값에 1을 덧셉하여 변경하고
      ...
      //만약 아이디가 일치하지 않은 데이터는 변경하지 말고 원래 데이터를
      //return
      return bucket;
    }); //end map
    //새로 생성된 bucketBody로 기존 bucketList배열을 교체
  ...
  //JSON 객체 의 속성변수 이름과 값이 일치 할때는 변수명만으로 JSON
  //객체를 선언하면 위의 코드와 같이 생성한다
  const functions = {
    bucket_delete,
    bucket_flag_toggle,
    bucket_complete,
    ...
    <>
      <div>
        <input
          ...
          className="w3-input
         w3-border w3-hover-gray w3-round"
        />
      </div>
      <div>
        <BucketList
